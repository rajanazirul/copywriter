...
const desiresCollectionQuery = query(desiresCollectionRef)

const desires = ref<FormData>()

const deleteContent = (id: any) => {
    deleteDoc(doc(desiresCollectionRef, id))
}
...
onMounted(() => {
    onSnapshot(desiresCollectionQuery, (querySnapshot) => {
        ...
        querySnapshot.forEach((doc: any) => {
            ...
            fbdesires.push(category)
        })
        desires.value = fbdesires
    })
})
</script>
...
<template>
    <div>
        <v-card color="#385F73" theme="dark">
            ...
            <v-card-subtitle>Ayat untuk menarik pembaca membeli produk anda.</v-card-subtitle>
        </v-card>
        ...
        <v-table>
            ...
            <tbody>
                <tr v-for="desire in desires" :key="desire.id">
                    <td>{{ desire.content }}</td>
                    <td>{{ desire.group }}</td>
                    <td>{{ desire.category }}</td>
                    <td>{{ desire.user }}</td>
                    <td>{{ desire.date }}</td>
                    <td><button type="button" @click="deleteContent(desire.id)"
